...
class FullImageTrending extends StatelessWidget {
  const FullImageTrending({super.key});

  @override
  Widget build(BuildContext context) {
    final images = context.read<TrendingBloc>().state.images;
    final maxCount = images.isNotEmpty
        ? images.fold(0, (maxCount, image) => max(maxCount, image.countSwap))
        : 0;
    return Scaffold(
      appBar: AppWidget.createSimpleAppBar(context: context, title: 'Trending'),
      body: Column(
        children: [
